...
class CSSDecoder(object):
    ...
    def __init__(self, d, Hx, Hz):
        mx, n = Hx.shape
        assert Hz.shape[1] == n
        mz, _ = Hz.shape

#        # Check operator adjacency matrix:
#        A = dot(Hx, Hx.transpose())
#        self.A = A
...
class LocalGaussianCSSDecoder(CSSDecoder):

    def decode(self, p, err, verbose=False, **kw):
        if verbose: print("error:   ", shortstr(err))
        s = self.check(err)
        if verbose: print("syndrome:", shortstr(s))
        #A = self.A
        m, n = self.m, self.n
...
#                cs.append(c)
        ...
        while cs:
            ...
            cs1 = []
            for c in cs:
                v = c.solve(self.Hz, verbose=False)
                if v is None:
                    cs1.append(c)
                else:
                    if (v0*v).max() > 0:
            ...
            if verbose: print("solution:", shortstr(v0))
